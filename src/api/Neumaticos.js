...
export const obtenerHistorialMovimientosPorCodigo = async (codigo) => {
  try {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/po-movimiento/historial-codigo/${codigo}`);
...
export const obtenerUltimosMovimientosPorPosicion = async (placa) => {
  try {
    ...
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/po-asignados/ultimo-movimiento/${placa}`);
    return response.data;
  } catch (error) {
...
export const getUltimaFechaInspeccionPorPlaca = async (placa) => {
  try {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/ultima-fecha-inspeccion-por-placa`, {
      params: { placa },
      withCredentials: true,
    });
...
export const obtenerCantidadNeumaticosBajaDefinitiva = async () => {
  const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/po-neumaticos/baja-definitiva/cantidad`, { withCredentials: true });
  return response.data.cantidad;
};

// Obtener la cantidad de neumáticos recuperados
export const obtenerCantidadNeumaticosRecuperados = async () => {
  const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/po-neumaticos/recuperados/cantidad`, { withCredentials: true });
  return response.data.cantidad;
};
...
export const desasignarConReemplazo = async (payload) => {
  try {
    const response = await axios.post(
      `${process.env.NEXT_PUBLIC_API_URL}/api/desasignar-con-reemplazo`,
